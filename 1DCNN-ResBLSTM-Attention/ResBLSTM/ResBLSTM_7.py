...
model_save_path = 'D:/human activity recognition/UCI-HAR/ResBLSTM_7/Res_BLSTM.hdf5'
...
def ResBLSTM(input):
    ...
    model = Bidirectional(LSTM(64, return_sequences=True))(y)
    model = Dropout(0.2)(model)
    model = LayerNormalization()(model)
    x_1 = Add()([y, model])

    model = Bidirectional(LSTM(64, return_sequences=True))(x_1)
    model = Dropout(0.2)(model)
    model = LayerNormalization()(model)
    y_1 = Add()([x_1, model])

    model = Bidirectional(LSTM(64, return_sequences=True))(y_1)
    model = Dropout(0.2)(model)
    model = LayerNormalization()(model)
    x_2 = Add()([y_1, model])

    model = Bidirectional(LSTM(64, return_sequences=True))(x_2)
    model = Dropout(0.2)(model)
    model = LayerNormalization()(model)
    y_2 = Add()([x_2, model])

    output = Bidirectional(LSTM(64, return_sequences=False))(y_2)
    return output
...
ResBLSTM_history_file = open('Res_BLSTM_7.json', 'w')
